...
class Facet:
    ...
    def __init__(self, vertexes):
        ...
        self.edges = [Edge(vertexes[i - 1],
                           vertexes[i]) for i in range(len(vertexes))]
    ...
    def area(self):
        ...
        for i in range(1, n-1):
            area += self.triangle_area(self.vertexes[0],
                                       self.vertexes[i], self.vertexes[i+1])
        return area

    def triangle_area(self, v1, v2, v3):
        a = v2 - v1
        b = v3 - v1
        vp = a.cross(b)  # векторное произведение
        return 0.5 * sqrt(vp.dot(vp))

    # Угол с горизонтальной плоскостью
    def angle_with_horizontal(self):
        # Вычисляем нормаль к грани
        normal = self.h_normal()
        # Скалярное произведение нормали и вертикального вектора
        dot_product = abs(normal.dot(Polyedr.V))
        # Длина нормали
        normal_length = sqrt(normal.x**2 + normal.y**2 + normal.z**2)
        # Угол между нормалью и вертикальным вектором
        return acos(dot_product / normal_length)

    # Грань вне единичного куба
    def is_outside_unit_cube(self):
        center = self.center()
        return not (-0.5 <= center.x <= 0.5
                    and -0.5 <= center.y <= 0.5
                    and -0.5 <= center.z <= 0.5)
    ...
    def draw(self, tk):  # pragma: no cover
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            for s in e.gaps:
                tk.draw_line(e.r3(s.beg)*self.c, e.r3(s.fin)*self.c)

    # Метод вычисления и печати требуемой площади граней
    def calculate_area(self):
        total_area = 0.0
        for facet in self.facets:
            if (facet.is_fully_invisible() and
                    facet.angle_with_horizontal() <= pi/7
                    and facet.is_outside_unit_cube()):
                total_area += facet.area()
        print(f"Суммарная площадь полностью невидимых граней: {total_area}")
