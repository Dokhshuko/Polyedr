...
4	1    4    8    5"""
    ...
    def test_angle_with_horizontal_horizontal(self):
        ...
        vertexes = [R3(0, 0, 0), R3(1, 0, 0), R3(1, 1, 0), R3(0, 1, 0)]
        facet = Facet(vertexes)
        expected_angle = 0.0  # Угол между нормалью горизонтальной грани и вертикалью равен 0
    ...
    def test_angle_with_horizontal_vertical(self):
        ...
        facet = Facet(vertexes)
        expected_angle = pi / 2  # Угол между нормалью вертикальной грани и вертикалью равен pi/2
        self.assertAlmostEqual(facet.angle_with_horizontal(), expected_angle)
    ...
    def test_angle_with_horizontal_45_degrees(self):
        ...
        facet = Facet(vertexes)
        expected_angle = pi / 4  # Угол между нормалью грани и вертикалью равен pi/4
        self.assertAlmostEqual(facet.angle_with_horizontal(), expected_angle, places=5)

    def test_is_outside_unit_cube_inside(self):
        # Грань, центр которой находится внутри единичного куба
        vertexes = [R3(-0.25, -0.25, -0.25), R3(0.25, -0.25, -0.25), R3(0.25, 0.25, -0.25), R3(-0.25, 0.25, -0.25)]
        facet = Facet(vertexes)
    ...
    def test_is_outside_unit_cube_outside(self):
        ...
        vertexes = [R3(1.0, 1.0, 1.0), R3(2.0, 1.0, 1.0), R3(2.0, 2.0, 1.0), R3(1.0, 2.0, 1.0)]
        facet = Facet(vertexes)
    ...
    def test_is_outside_unit_cube_on_edge(self):
        ...
        vertexes = [R3(-0.5, -0.5, 0), R3(0.5, -0.5, 0), R3(0.5, 0.5, 0), R3(-0.5, 0.5, 0)]
        facet = Facet(vertexes)
        self.assertFalse(facet.is_outside_unit_cube())
    ...
    def test_is_fully_invisible_some_visible(self):
        ...
        vertexes = [R3(0, 0, 0), R3(1, 0, 0), R3(1, 1, 0)]
        facet = Facet(vertexes)
        facet.edges[0].gaps = [Segment(0, 0.5)]  # Первое ребро частично видимо
    ...
    def test_is_fully_invisible_all_visible(self):
        ...
        facet = Facet(vertexes)
        for edge in facet.edges:
            edge.gaps = [Segment(0, 1)]  # Все сегменты невырожденные, т.е. рёбра видимы
        self.assertFalse(facet.is_fully_invisible())

    def test_last(self):
        self.assertAlmostEqual(Polyedr('data/test1.geom').calculate_area(), 0.0)
